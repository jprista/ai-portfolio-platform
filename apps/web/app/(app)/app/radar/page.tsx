    ...
    <div className="mx-auto max-w-[1240px] px-10 py-9">
      <header className="mb-3">
        ...
        <p className="mt-1 max-w-[760px] text-[13px] leading-relaxed text-muted">
          O que as maiores gestoras do mundo <b>detêm e mudaram</b> — direto das declarações
          regulatórias públicas (SEC 13F). Evidência para a sua decisão; a recomendação é sempre sua.
        </p>
      </header>
      <p className="mb-7 inline-block rounded-lg bg-gold-soft px-3 py-1.5 text-[11.5px] text-ink/70">
        Dados com defasagem regulatória (~45 dias após o fechamento do trimestre) — declarada por desenho.
        Gestoras brasileiras (carteiras CVM) entram na próxima versão.
      </p>
          ...
          <Card key={m.name} hover className="p-5">
            <div className="flex items-baseline justify-between border-b border-hairline pb-3">
              ...
              <div className="text-[11px] text-faint">
                13F · {m.period_end.split("-").reverse().join("/")}
              </div>
            </div>
                ...
                <div key={h.rank} className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <span className="truncate text-[13px] font-medium text-ink">{h.issuer_name}</span>
            ...
            {m.source_url && (
              ...
              >
                Fonte: declaração original na SEC ↗
              </a>
            )}
          </Card>
        ))}
